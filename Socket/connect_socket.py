from Socket.socket import socketio,emit
...
def disconnect():
    ...
    for room_name, room_data in room_data_manager._data_store.items():
        if 'user' in room_data and request.sid in room_data['user']:
            ...
            if user_name:
                socket_class.update_room_player_count(room_name, "님이 퇴장 하셨습니다.", user_name)  # 플레이어 수 업데이트
            if not room_data['user']:  # 방에 더 이상 유저가 없으면 방 제거
                ...
                if room_data_manager._data_store[room_name]['room_info']['room_status']:
                    ...
                    if user_name in socket_class.voted_users[room_name]:
                        socket_class.vote_counts[room_name] = vote_counts[room_name] - 1
