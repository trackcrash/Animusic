...
import random
import threading
def update_room_player_count(room_key, msg, player_name, color, effect):
    player_count= len(room_data_manager._data_store[room_key]['user'])
    player = player_name
    msg_color = color
    players = room_data_manager._data_store[room_key]['user']
    bool_effect = effect;
    emit('room_players_update', {'room_key': room_key, 'player_count':player_count,'player' : player, 'players': players, 'msg': msg, 'color': msg_color,'effect': bool_effect}, broadcast=True)
